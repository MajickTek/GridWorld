...
public class PseudoInfiniteViewport extends JViewport {
	...
	public void setViewPosition(Point pt) {
		final boolean isAdjusting = scrollParent.getVerticalScrollBar().getValueIsAdjusting()
				|| scrollParent.getHorizontalScrollBar().getValueIsAdjusting();
		boolean changed = true;

		if (viewIsUnbounded()) {
			final int hDelta = pt.x - panPoint.x;
			final int vDelta = pt.y - panPoint.y;
			if ((hDelta != 0 && vDelta == 0) || (vDelta != 0 && hDelta == 0)) {
