...
public class ImageDisplay extends AbstractDisplay {
	...
	private final Map<String, Image> tintedVersions = new LinkedHashMap<>(128, 0.75F, true) {
	...
	public ImageDisplay(Class<?> cl) throws IOException {
		...
		final URL url = cl.getClassLoader().getResource(imageFilename + imageExtension);
	...
	public void draw(Object obj, Component comp, Graphics2D g2) {
		...
		if (tinted == null) // not cached, need new filter for color
		{
			...
			{
				try {
					final URL url = cl.getClassLoader().getResource(imageFilename + imageSuffix + imageExtension);
					if (url == null) {
			...
			if (color == null) {
				...
				final FilteredImageSource src = new FilteredImageSource(untinted.getSource(), new TintFilter(color));
