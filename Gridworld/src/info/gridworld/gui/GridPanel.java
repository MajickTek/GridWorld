...
public class GridPanel extends JPanel implements Scrollable, PseudoInfiniteViewport.Pannable {
	...
	private void drawOccupant(Graphics2D g2, int xleft, int ytop, Object obj) {
		final Rectangle cellToDraw = new Rectangle(xleft, ytop, cellSize, cellSize);
		...
		if (cellToDraw.intersects(g2.getClip().getBounds())) {
			...
			final Display displayObj = displayMap.findDisplayFor(obj.getClass());
	...
	private void drawGridlines(Graphics2D g2) {
		...
		final int top = getInsets().top, left = getInsets().left;

		final int miny = Math.max(0, (curClip.y - top) / (cellSize + 1)) * (cellSize + 1) + top;
		final int minx = Math.max(0, (curClip.x - left) / (cellSize + 1)) * (cellSize + 1) + left;
		final int maxy = Math.min(numRows, (curClip.y + curClip.height - top + cellSize) / (cellSize + 1))
				* (cellSize + 1) + top;
		final int maxx = Math.min(numCols, (curClip.x + curClip.width - left + cellSize) / (cellSize + 1))
				* (cellSize + 1) + left;

		g2.setColor(Color.GRAY);
		for (int y = miny; y <= maxy; y += cellSize + 1) {
			for (int x = minx; x <= maxx; x += cellSize + 1) {
				final Location loc = locationForPoint(new Point(x + cellSize / 2, y + cellSize / 2));
	...
	private void drawOccupants(Graphics2D g2) {
		final ArrayList<Location> occupantLocs = grid.getOccupiedLocations();
		for (final Location occupantLoc : occupantLocs) {
			final Location loc = occupantLoc;

			final int xleft = colToXCoord(loc.getCol());
			final int ytop = rowToYCoord(loc.getRow());
	...
	private void drawCurrentLocation(Graphics2D g2) {
		...
		if (currentLocation != null) {
			final Point p = pointForLocation(currentLocation);
	...
	private void drawWatermark(Graphics2D g2) {
		final String versionId = resources.getString("version.id");
		...
		final Rectangle2D bounds = g2.getFont().getStringBounds(versionId, frc);
		...
		final LineMetrics lm = g2.getFont().getLineMetrics(versionId, frc);
	...
	public void setGrid(Grid<?> gr) {
		...
		final JViewport vp = getEnclosingViewport(); // before changing, reset
		// scroll/pan position
		if (vp != null) {
	...
	public void recenter(Location loc) {
		...
		repaint();
		final JViewport vp = getEnclosingViewport();
		if (vp != null) {
	...
	public String getToolTipText(MouseEvent evt) {
		final Location loc = locationForPoint(evt.getPoint());
	...
	public void moveLocation(int dr, int dc) {
		final Location newLocation = new Location(currentLocation.getRow() + dr, currentLocation.getCol() + dc);
		...
		final JViewport viewPort = getEnclosingViewport();
		if (isPannableUnbounded()) {
			...
			final int rows = dim.height / (cellSize + 1);
			final int cols = dim.width / (cellSize + 1);
			...
			int dy = 0;
			final Point p = pointForLocation(currentLocation);
			final Rectangle locRect = new Rectangle(p.x - cellSize / 2, p.y - cellSize / 2, cellSize + 1, cellSize + 1);
	...
	private void recalculateCellSize(int minSize) {
		if (numRows == 0 || numCols == 0) {
			...
			final JViewport vp = getEnclosingViewport();
			final Dimension viewableSize = (vp != null) ? vp.getSize() : getSize();
			final int desiredCellSize = Math.min((viewableSize.height - extraHeight()) / numRows,
	...
	public void showPanTip() {
		...
		final Location loc = locationForPoint(upperLeft);
		if (loc != null) {
