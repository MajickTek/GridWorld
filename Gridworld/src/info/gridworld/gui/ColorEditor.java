...
public class ColorEditor extends PropertyEditorSupport {
	...
	public Object getValue() {
		final ColorIcon value = (ColorIcon) combo.getSelectedItem();
	...
	private static class SolidColorIcon implements ColorIcon {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			final Rectangle r = new Rectangle(x, y, WIDTH - 1, HEIGHT - 1);
	...
	private static class RandomColorIcon implements ColorIcon {
		...
		public void paintIcon(Component c, Graphics g, int x, int y) {
			...
			final Graphics2D g2 = (Graphics2D) g;
			final Color oldColor = g2.getColor();
			final Rectangle r1 = new Rectangle(x, y, WIDTH / 4, HEIGHT - 1);
			for (int i = 0; i < 4; i++) {
	...
	private final JComboBox<ColorIcon> combo;
